...
class VideoProcessor:
    ...
    def process_video(self, input_video_path, output_video_path):
        ...
        if not cap.isOpened():
            raise ValueError(f"Cannot open {input_video_path}")
        ...
        while True:
            ...
            result = self.classifier.predict(rgb_frame)
            ...
            is_polyp = self.classifier.is_polyp(rgb_frame)

            if is_polyp:
                polyp_frames += 1

                mask, overlay = self.segmenter.predict_and_overlay(rgb_frame)
            ...
            status = "POLYP DETECTED" if is_polyp else "NO POLYP"
            ...
            status_color = (0, 255, 0) if is_polyp else (0, 0, 255)
        ...
        return {
            ...
            "polyp_percentage": (polyp_frames / max(processed_frames, 1)) * 100,
            "class_counts": class_counts,
            "dominant_class": dominant_class,
            "average_confidence": float(np.mean(confidence_history)),
            "median_confidence": float(np.median(confidence_history)),
